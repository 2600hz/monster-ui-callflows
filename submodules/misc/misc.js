  ...
  var app = {
    ...
    miscDefineActions: function (args) {
      ...
      $.extend(callflow_nodes, {
            ...
            self.callApi({
              ...
              success: function (data, status) {
                ...
                $.each(data.data, function () {
                  if (!this.featurecode && this.id !== self.flow.id) {
                    this.name = this.name
                      ...
                      : self.i18n.active().oldCallflows.no_numbers;
                ...
                $("#add", popup_html).click(function () {
                  node.setMetadata(
                    "id",
                    $("#object-selector", popup_html).val()
                  );

                  node.caption = $(
                    "#object-selector option:selected",
                    popup_html
                  ).text();
        ...
        "prepend_cid[action=prepend]": {
          ...
          caption: function (node) {
            return (
              (node.getMetadata("caller_id_name_prefix") || "") +
              " " +
              (node.getMetadata("caller_id_number_prefix") || "")
            );
          },
          edit: function (node, callback) {
        ...
        "manual_presence[]": {
          ...
          edit: function (node, callback) {
            ...
            $("#add", popup_html).click(function () {
              ...
              node.setMetadata(
                "status",
                $("#presence_status option:selected", popup_html).val()
              );
                ...
                self.getTemplate({
                  ...
                  data: {
                    data: {
                      items: results,
                      selected:
                        node.getMetadata("device_id") ||
                        node.getMetadata("group_id") ||
                        node.getMetadata("user_id") ||
                        "",
                    },
                  },
                  submodule: "misc",
        ...
        "receive_fax[]": {
          ...
          edit: function (node, callback) {
            self.miscUserList(function (data, status) {
                ...
                self.getTemplate({
                  ...
                  data: {
                    objects: {
                      ...
                      t_38:
                        node.getMetadata("media") &&
                        (node.getMetadata("media").fax_option || false),
                    },
                  },
                  submodule: "misc",
                })
              ...
              if (
                $("#user_selector option:selected", popup_html).val() ===
                undefined
              ) {
        ...
        "pivot[]": {
          ...
          edit: function (node, callback) {
            ...
            $("#add", popup_html).click(function () {
              node.setMetadata(
                "voice_url",
                $("#pivot_voiceurl_input", popup_html).val()
              );
              node.setMetadata(
                "method",
                $("#pivot_method_input", popup_html).val()
              );
              node.setMetadata(
                "req_format",
                $("#pivot_format_input", popup_html).val()
              );
          ...
          edit: function (node, callback) {
              ...
              self.getTemplate({
                ...
                data: {
                  data_disa: {
                    ...
                    use_account_caller_id: node.getMetadata(
            ...
            $("#add", popup_html).click(function () {
              var save_disa = function () {
                ...
                setData(
                  "interdigit",
                  $("#disa_interdigit_input", popup_html).val()
                );
                setData(
                  "preconnect_audio",
                  $("#preconnect_audio", popup_html).val()
                );
                setData(
                  "use_account_caller_id",
                  !$("#disa_keep_original_caller_id", popup_html).is(":checked")
                );
                setData(
                  "max_digits",
                  $("#disa_max_digits_input", popup_html).val()
                );
              ...
              if ($("#disa_pin_input", popup_html).val() === "") {
                monster.ui.confirm(
                  self.i18n.active().oldCallflows.not_setting_a_pin,
                  function () {
                    save_disa();
                  }
                );
        ...
        "collect_dtmf[]": {
          ...
          edit: function (node, callback) {
            ...
            popup_html = $(
              self.getTemplate({
                ...
                data: {
                  data_dtmf: {
                    interdigit_timeout:
                      node.getMetadata("interdigit_timeout") || "",
            ...
            $("#add", popup_html).click(function () {
              ...
              setData(
                "interdigit_timeout",
                $("#collect_dtmf_interdigit_input", popup_html).val()
              );
              setData(
                "collection_name",
                $("#collect_dtmf_collection_input", popup_html).val()
              );
              setData(
                "max_digits",
                $("#collect_dtmf_max_digits_input", popup_html).val()
              );
              setData(
                "terminator",
                $("#collect_dtmf_terminator_input", popup_html).val()
              );
              setData(
                "timeout",
                $("#collect_dtmf_timeout_input", popup_html).val()
              );
        ...
        "response[]": {
          ...
          edit: function (node, callback) {
            self.miscMediaList(function (data) {
              ...
              if (
                $("#media_selector option:selected", popup_html).val() ===
                  undefined ||
                $("#media_selector option:selected", popup_html).val() ===
                  "null"
              ) {
              ...
              $("#add", popup_html).click(function () {
                if (
                  $("#response_code_input", popup_html)
                    .val()
                    .match(/^[1-6][0-9]{2}$/)
                ) {
                  node.setMetadata(
                    "code",
                    $("#response_code_input", popup_html).val()
                  );
                  node.setMetadata(
                    "message",
                    $("#response_message_input", popup_html).val()
                  );
                  if (
                    $("#media_selector", popup_html).val() &&
                    $("#media_selector", popup_html).val() !== "null"
                  ) {
                    node.setMetadata(
                      "media",
                      $("#media_selector", popup_html).val()
                  ...
                  monster.ui.alert(
                    "error",
                    self.i18n.active().oldCallflows
                      .please_enter_a_valide_sip_code
                  );
                }
              });
        ...
        "json_editor[]": {
          ...
          edit: function (node, callback) {
            if (
              _.isEmpty(self.appFlags.misc.jsonEditor.unsupportedCallflowsList)
            ) {
              self.miscSchemasList(function (data) {
                var supportedModules = _.chain(monster.apps.callflows.actions)
                    .map("module")
                    .uniq()
                    .filter(_.isString)
                    .value(),
                  getCallflowModules = _.chain(data)
                    ...
                    .value();
                unsupportedModules = _.difference(
                  getCallflowModules,
                  supportedModules
                );

                self.appFlags.misc.jsonEditor.unsupportedCallflowsList =
                  unsupportedModules;
    ...
    miscEditMissedCallAlerts: function (node, callback) {
      ...
      self.miscUserList(function (users) {
        ...
        var template = $(
          ...
          widget = monster.ui.linkedColumns(
            template.find(".items-selector-wrapper"),
            items,
            selectedItems,
            {
              i18n: {
                columnsTitles: {
                  available:
                    self.i18n.active().callflows.missedCallAlert
                      .unselectedUsers,
                  selected:
                    self.i18n.active().callflows.missedCallAlert.selectedUsers,
                },
              },
    ...
    miscEditSetCAV: function (node, callback) {
      var self = this,
        ...
        addRow = function (template, data) {
          ...
          template.find(".cav-remove-row").on("click", function () {
    ...
    miscRenderEditWebhook: function (node, callback) {
      var self = this,
        ...
        initTemplate = function () {
          var data = {
              hasVerbWithFormat: _.includes(
                self.appFlags.misc.webhook.verbsWithFormat,
                node.getMetadata("http_verb")
              ),
              bodyFormatList: _.map(
                self.appFlags.misc.webhook.bodyFormats,
                function (item) {
                  return {
                    value: item,
                    label: _.get(
                      self.i18n.active().callflows.webhook.format.options,
                      _.camelCase(item),
                      monster.util.formatVariableToDisplay(item)
                    ),
                  };
                }
              ),
          ...
          $template.find("#add").on("click", function (e) {
            ...
            _.each(formData, function (value, key) {
              if (key === "custom_data") {
                value = _.chain(value).keyBy("key").mapValues("value").value();
              } else if (key === "retries") {
      ...
      var self = this,
        ...
        initTemplate = function () {
          var $template = $(
            ...
            jsoneditor = monster.ui.jsoneditor(
              $target,
              options,
              node.data.data
            );
    ...
    miscSetSchema: function (template, jsoneditor, callback) {
      var self = this,
        ...
        callflowSchema =
          self.appFlags.misc.jsonEditor.callflowsListSchema[selectedOption];
      ...
      if (callflowSchema) {
        ...
        $template.find("#name").prop("disabled", true);
        ...
        self.miscGetSchema({
          ...
          //making async call to wait for getSchema to finish
          success: async function (data) {
            //validate if schema has reference to other schemas
            var refsList = self.validateSubSchema(data);

            //if so, get those schemas and set them in jsoneditor
            var Refs = refsList ? await self.getSubSchema(refsList) : {};

            //set schema to validate schema
            jsoneditor.setSchema(data, Refs);
            ...
            $template.find("#name").prop("disabled", false);
    ...
    validateSubSchema: function (schema) {
      /*validate accordint to the path where the ref can be found
      and return a list with their values*/
      if (_.has(schema, "properties.config.$ref")) {
        ...
        return schema.properties.macros.items.oneOf.map(function (item) {
          return item["$ref"];
        });
      }
      /*if no ref is found, return null*/
      return false;
    },

    /*
    @param reflist: array with the value of the reference to other schemas
    return a promise in order to wait for all the schemas to be fetched */
    getSubSchema: function (refsList) {
      var self = this;
      var Refs = {};

      var subSchemaPromise = new Promise((resolve) => {
        refsList.forEach(function (ref, index) {
          self.miscGetSchema({
            ...
            success: function (data) {
              Refs = { ...Refs, [data.id]: data };
              //resolve when all the schemas are fetched
