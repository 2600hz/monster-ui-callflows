		...
		groupsDefineActions: function(args) {
			...
			$.extend(callflow_nodes, {
				...
				'missed_call_alert[]': {
					...
					edit: function(node, callback) {
						self.groupsEditMissedCallAlerts(node, callback);
					}
				}
			});
		},

		groupsEditMissedCallAlerts: function(node, callback) {
			var self = this,
				popup,
				popup_html,
				selected_users = node.getMetadata('recipients'),
				mapUsers = _.indexBy(selected_users, 'id'),
				selected_endpoints = {},
				unselected_users = [];

			self.groupsUserList(function(users) {
				_.each(users, function(user) {
					...
					if (mapUsers.hasOwnProperty(user.id)) {
						selected_endpoints[user.id] = formattedUser;
					} else {
						unselected_users.push(formattedUser);
					}
				});
				unselected_users = _.sortBy(unselected_users, 'name');

				popup_html = $(monster.template(self, 'groups-missedCallAlert-dialog'));

				$.each(unselected_users, function() {
					$('#users_pane .connect.left', popup_html).append($(monster.template(self, 'groups-missedCallAlert-element', this)));
				});

				$.each(selected_endpoints, function() {
					$('.connect.right', popup_html).append($(monster.template(self, 'groups-missedCallAlert-element', this)));
				});
				...
				$('#save_missed_call_alerts', popup_html).click(function() {
					var recipients = [];

					$('.right .connect li', popup_html).each(function() {
						recipients.push({ type: 'user', id: $(this).data('id') });
					});
				...
				var remove_element = function(li) {
					...
					data.name = $.trim($('.item_name', $parent_li).html());
					$('#users_pane .connect.left', popup_html).append($(monster.template(self, 'groups-missedCallAlert-element', data)));
					$parent_li.remove();
					...
					if (data.name.toLowerCase().indexOf($('#users_pane .searchfield', popup_html).val().toLowerCase()) === -1) {
